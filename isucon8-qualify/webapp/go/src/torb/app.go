...
func getEvents(all bool) ([]*Event, error) {
	...
	for rows.Next() {
		...
		var event Event
		err := rows.Scan(&event.ID, &event.Title, &event.PublicFg, &event.ClosedFg, &event.Price)
	...
	mapEventSheetToReservation := make(map[int64]map[int64]Reservation)
	eventIDStrs := make([]string, 0, len(events))

	for _, event := range events {
		mapEventSheetToReservation[event.ID] = make(map[int64]Reservation)
		eventIDStrs = append(eventIDStrs, strconv.FormatInt(event.ID, 10))
	}

	rows, err = db.Query("SELECT event_id, sheet_id, user_id, reserved_at FROM reservations WHERE event_id IN (" + strings.Join(eventIDStrs, ",") + ") AND canceled_at IS NULL")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		r, ok := mapEventSheetToReservation[reservation.EventID][reservation.SheetID]
		if ok && r.ReservedAt.Before(*reservation.ReservedAt) {
			continue
		} else {
			mapEventSheetToReservation[reservation.EventID][reservation.SheetID] = reservation
		}
	}
	...
	for i, event := range events {
		...
		for _, sheet := range sheets {
			...
			r, ok := mapEventSheetToReservation[event.ID][sheet.ID]
			if ok {
				sheet.Mine = false
				sheet.Reserved = true
...
func getEvent(eventID, loginUserID int64) (*Event, error) {
	...
	for rows.Next() {
		...
		r, ok := mapEventSheetToReservation[event.ID][reservation.SheetID]
		if ok && r.ReservedAt.Before(*reservation.ReservedAt) {
			continue
		} else {
			mapEventSheetToReservation[event.ID][reservation.SheetID] = reservation
		}
	}
...
func main() {
	...
	e.POST("/api/users", func(c echo.Context) error {
		...
		var uid int64
		if err := tx.QueryRow("SELECT id FROM users WHERE login_name = ?", params.LoginName).Scan(&uid); err != sql.ErrNoRows {
			tx.Rollback()
			if err == nil {
	...
	e.GET("/api/users/:id", func(c echo.Context) error {
		...
		rows, err := db.Query("SELECT r.id, r.event_id, r.sheet_id, r.user_id, r.reserved_at, r.canceled_at, s.rank, s.num FROM reservations r INNER JOIN sheets s ON s.id = r.sheet_id WHERE r.user_id = ? ORDER BY r.reserved_at DESC LIMIT 5", user.ID)
		if err != nil {
			return err
		}
		defer rows.Close()
		...
		for rows.Next() {
			var reservation Reservation
			var sheet Sheet
			err := rows.Scan(&reservation.ID, &reservation.EventID, &reservation.SheetID, &reservation.UserID, &reservation.ReservedAt, &reservation.CanceledAt, &sheet.Rank, &sheet.Num)
			if err != nil {
				return err
			}
	...
	e.POST("/api/events/:id/actions/reserve", func(c echo.Context) error {
		...
		for {
			var minID, maxID, randID int64 = 1, 1, 1
			//err := db.QueryRow("SELECT * FROM sheets WHERE id NOT IN (SELECT sheet_id FROM reservations WHERE event_id = ? AND canceled_at IS NULL FOR UPDATE) AND `rank` = ? ORDER BY RAND() LIMIT 1", event.ID, params.Rank).Scan(&sheet.ID, &sheet.Rank, &sheet.Num, &sheet.Price)
			err := db.QueryRow("SELECT MIN(id), MAX(id) FROM sheets").Scan(&minID, &maxID)

			for {
				rand.Seed(time.Now().UnixNano())
				randID = int64(float64(maxID-minID)*rand.Float64()) + minID
				var s, n sql.NullInt64

				err = db.QueryRow("SELECT id, num FROM sheets WHERE id >= ? AND id NOT IN (SELECT sheet_id FROM reservations WHERE event_id = ? AND canceled_at IS NULL) AND `rank` = ? LIMIT 1", randID, event.ID, params.Rank).Scan(&s, &n)
				if s.Valid {
					sheet.ID = s.Int64
					sheet.Num = n.Int64
					break
	...
	e.GET("/admin/api/reports/events/:id/sales", func(c echo.Context) error {
		...
		rows, err := db.Query("SELECT r.id, r.user_id, r.reserved_at, r.canceled_at, s.rank, s.num, s.price, e.price FROM reservations r INNER JOIN sheets s ON s.id = r.sheet_id INNER JOIN events e ON e.id = r.event_id WHERE r.event_id = ? ORDER BY reserved_at ASC FOR UPDATE", event.ID)
		if err != nil {
			return err
		}
		defer rows.Close()
		...
		for rows.Next() {
			...
			//err := rows.Scan(&reservation.ID, &reservation.EventID, &reservation.SheetID, &reservation.UserID, &reservation.ReservedAt, &reservation.CanceledAt, &sheet.Rank, &sheet.Num, &sheet.Price, &event.Price)
			err := rows.Scan(&reservation.ID, &reservation.UserID, &reservation.ReservedAt, &reservation.CanceledAt, &sheet.Rank, &sheet.Num, &sheet.Price, &event.Price)
			if err != nil {
				return err
			}
			report := Report{
	...
	e.GET("/admin/api/reports/sales", func(c echo.Context) error {
		...
		rows, err := db.Query("SELECT r.id, r.user_id, r.reserved_at, r.canceled_at, s.rank, s.num, s.price, e.id, e.price FROM reservations r INNER JOIN sheets s ON s.id = r.sheet_id INNER JOIN events e ON e.id = r.event_id ORDER BY reserved_at ASC FOR UPDATE")
		if err != nil {
			return err
		}
		defer rows.Close()
		...
		for rows.Next() {
			...
			var sheet Sheet
			var event Event
			//err := rows.Scan(&reservation.ID, &reservation.EventID, &reservation.SheetID, &reservation.UserID, &reservation.ReservedAt, &reservation.CanceledAt, &sheet.Rank, &sheet.Num, &sheet.Price, &event.ID, &event.Price)
			err := rows.Scan(&reservation.ID, &reservation.UserID, &reservation.ReservedAt, &reservation.CanceledAt, &sheet.Rank, &sheet.Num, &sheet.Price, &event.ID, &event.Price)
			if err != nil {
				return err
			}
			report := Report{
